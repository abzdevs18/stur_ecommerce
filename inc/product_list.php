<?php
    ...
    if ($_POST['query'] == 'top_rated') {
        ...
        // $q_b = mysqli_query($conn,"SELECT * FROM product WHERE EXISTS ( SELECT * FROM rating WHERE product_fk = product.p_k)");
    } elseif ($_POST['query'] == 'new_ar') {
        $q_b = mysqli_query($conn,"SELECT * FROM product ORDER BY timestamp DESC LIMIT 4");
    } elseif ($_POST['query'] == 'best_ar') {
        $q_b = mysqli_query($conn,"SELECT * FROM product WHERE EXISTS ( SELECT * FROM orders WHERE product_fk = product.p_k)");
    } 
    // else {
    //     $id = $_POST['query'];
    //     $q_b = mysqli_query($conn,"SELECT * FROM product WHERE category = '$id'");
    // }
